...
/*
 * A V4L2 driver for VideoBrick ADV7611 Chip cameras.
 *
 */
...
MODULE_AUTHOR("Georg Ottinger");
MODULE_DESCRIPTION("A low-level driver for VideoBrick ADV7611 HDMI Receiver");
MODULE_LICENSE("GPL");

//for internel driver debug
#define DEV_DBG_EN   		1
#if(DEV_DBG_EN == 1)
#define csi_dev_dbg(x,arg...) printk(KERN_INFO"[CSI_DEBUG][ADV7611]"x,##arg)
#else
#define csi_dev_dbg(x,arg...)
#endif
#define csi_dev_err(x,arg...) printk(KERN_INFO"[CSI_ERR][ADV7611]"x,##arg)
#define csi_dev_print(x,arg...) printk(KERN_INFO"[CSI][ADV7611]"x,##arg)
...
#define V4L2_IDENT_SENSOR 0x2051
...
unsigned char adv7611_i2c_map[7] = {
	0x40, // ADV7611_MAP_CEC 
	0x38, // ADV7611_MAP_INFOFRAME
	0x26, // ADV7611_MAP_DPLL
	0x32, // ADV7611_MAP_KSV
	0x36, // ADV7611_MAP_EDID
	0x34, // ADV7611_MAP_HDMI
	0x22, //ADV7611_MAP_CP
};
...
//	{ ADV7611_IO_PRIMARY_MODE, 0x06 }, 	// V-FREQ = 60Hz Prim_Mode = HDMI-GR 
	...
	{ ADV7611_IO_REG_15, 0x80 }, 		// Disable Tristate of Pins (Audio output pins active)
	{ ADV7611_IO_LLC_DLL, 0x83	}, 		// LLC DLL phase 
...
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	return adv7611_read(sd,client->addr,(unsigned char)reg,data,len);
}

static int adv7611_cec_read(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *data, unsigned char len)
{
	return adv7611_read(sd,adv7611_i2c_map[ADV7611_MAP_CEC],reg,data,len);
}

static int adv7611_infoframe_read(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *data, unsigned char len)
{
	return adv7611_read(sd,adv7611_i2c_map[ADV7611_MAP_INFOFRAME],reg,data,len);
}

static int adv7611_dpll_read(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *data, unsigned char len)
{
	return adv7611_read(sd,adv7611_i2c_map[ADV7611_MAP_DPLL],reg,data,len);
}

static int adv7611_ksv_read(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *data, unsigned char len)
{
	return adv7611_read(sd,adv7611_i2c_map[ADV7611_MAP_KSV],reg,data,len);
}
...
static int adv7611_edid_read(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *data, unsigned char len)
{
	return adv7611_read(sd,adv7611_i2c_map[ADV7611_MAP_EDID],reg,data,len);
}
...
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	return adv7611_write(sd,client->addr,(unsigned char)reg,data,len);
}

static int adv7611_cec_write(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *data, unsigned char len)
{
	return adv7611_write(sd,adv7611_i2c_map[ADV7611_MAP_CEC],reg,data,len);
}

static int adv7611_infoframe_write(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *data, unsigned char len)
{
	return adv7611_write(sd,adv7611_i2c_map[ADV7611_MAP_INFOFRAME],reg,data,len);
}

static int adv7611_dpll_write(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *data, unsigned char len)
{
	return adv7611_write(sd,adv7611_i2c_map[ADV7611_MAP_DPLL],reg,data,len);
}
...
static int adv7611_edid_write(struct v4l2_subdev *sd, unsigned char reg,
		unsigned char *data, unsigned char len)
{
	return adv7611_write(sd,adv7611_i2c_map[ADV7611_MAP_EDID],reg,data,len);
}
...
static int adv7611_init(struct v4l2_subdev *sd)
{
	...
	for(i = 0; i < n; i++)
	{
		ret = adv7611_io_write(sd, defs_adv7611_io[i].reg, &(defs_adv7611_io[i].value),1);
		if(ret < 0)
			return ret;
	}
...
/**************
* sensor_power needs to be implemented using I2C
* POWER_DOWN
**************/
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	ret = adv7611_io_read(sd, ADV7611_IO_RD_INFO, data, 2);
	if (ret < 0) {
		csi_dev_err("sensor_read err at sensor_detect!\n");
		return ret;
	}

	csi_dev_print("Read Chip ID: 0x%02x%02x!\n", data[0], data[1]);

/* HACK-WARNING: Chip ID is hardcoded here */
	if(data[0] != 0x20 || data[1] != 0x51)
		return -ENODEV;
...
/* HACK-Warning: sun4i_csi1 expects h/vflip to be present somehow */
	case V4L2_CID_VFLIP:
...
//	int ret;
	csi_dev_dbg("entered sensor_probe.\n");
